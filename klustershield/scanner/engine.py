...
<body>
    <h1>🛡️ KlusterShield Compliance Report</h1>
    <div class="meta">
    ...
    <div class="footer">
        Generated by <a href="https://github.com/rudy101/KlusterShield" style="color:#60a5fa">KlusterShield</a>
        — Apache 2.0 License
    </div>
</body>
</html>
...
class ComplianceScanner:
    ...
    def scan(self, namespace: str | None = None) -> ScanResult:
        ...
        checks: list[tuple[str, Callable[..., ControlCheck]]] = [
            ...
            ("PO.1.1 – Security requirements defined", self._check_pod_security_admission),
            ("PO.3.2 – Toolchain controls in place", self._check_network_policies_exist),
            # PS — Protect Software
            ("PS.1.1 – Privileged containers blocked", self._check_no_privileged_containers),
            ("PS.2.1 – Root containers blocked", self._check_no_root_containers),
            # PW — Produce Well-Secured Software
            ("PW.4.1 – Resource limits enforced", self._check_resource_limits),
            ("PW.7.2 – Read-only root filesystem", self._check_readonly_root_fs),
            # RV — Respond to Vulnerabilities
            ("RV.1.3 – Image tags pinned (no :latest)", self._check_no_latest_tags),
            ("RV.3.3 – hostPID/hostNetwork disabled", self._check_host_namespaces),
            # DS — Distribute Securely
            ("DS.1.1 – Default deny network policy", self._check_default_deny_policy),
            ("DS.2.1 – Secrets not in env vars", self._check_secrets_not_in_env),
            # PV — Verify with Automated Testing
            ("PV.1.1 – RBAC enabled and configured", self._check_rbac_configured),
            ("PV.1.2 – ServiceAccount tokens not auto-mounted", self._check_service_account_tokens),
        ]
    ...
    def _check_rbac_configured(self, namespace: str | None) -> ControlCheck:
        ...
        if roles:
            ...
            check.status = Status.WARN
            check.finding = "No explicit RBAC Roles found — relying on ClusterRoles only"
            check.remediation = "Run: klustershield provision --namespace <ns>"
        return check
